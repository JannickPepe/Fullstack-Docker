...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
    ...
    return (
        <div className={`user-interface ${bgColor} ${backendName} mx-auto w-full max-w-[1280px] p-4 my-4 rounded shadow`}>
            ...
            <div className='lg:flex lg:justify-center lg:gap-8'>
                ...
                <form onSubmit={createUser} className="mb-6 p-4 bg-blue-100 rounded shadow max-w-lg">
                    <input
                        placeholder="Name"
                        value={newUser.name}
                    ...
                    <input
                        ...
                        className="mb-2 w-full p-2 border border-gray-300 rounded"
                    />
                    <button type="submit" className="w-full p-2 text-white bg-indigo-600 rounded hover:bg-blue-600">
                        Add User
                    </button>
                </form>

                {/* Form to update user */}
                <form onSubmit={handleUpdateUser} className="mb-6 p-4 bg-blue-100 rounded shadow max-w-lg">
                    <input
                        placeholder="User ID"
                        value={updateUser.id}
                    ...
                    <input
                        ...
                        className="mb-2 w-full p-2 border border-gray-300 rounded"
                    />
                    <button type="submit" className="w-full p-2 text-white bg-violet-500 rounded hover:bg-green-600">
                        Update User
                    </button>
                </form>
            </div>
            ...
            {/* Display users */}
            <div className="lg:grid lg:grid-cols-2 lg:gap-8 mt-8 px-2 lg:px-6 space-y-4 lg:space-y-0">
                {users.map((user) => (
                    <div key={user.id} className="py-4 px-4 rounded-lg shadow bg-indigo-400">
